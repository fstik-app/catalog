...
import WorkerConnector from '../telegram-web/WorkerConnector';
import { animate } from '../telegram-web/animation';
import { generateIdFor, cycleRestrict } from '../utils';

import { IS_SINGLE_COLUMN_LAYOUT, IS_ANDROID, IS_SAFARI, DPR } from '@/shared/environment';
import RlottieWorker from '@/shared/lib/rlottie/rlottie.worker?worker';
import FetchWorker from '@/shared/lib/fetch-worker/shared.worker?worker';
...
const CHUNK_SIZE = 1;
const MAX_WORKERS = 4;
const HIGH_PRIORITY_QUALITY = IS_SINGLE_COLUMN_LAYOUT ? 0.5 : 0.75;
const LOW_PRIORITY_QUALITY = IS_ANDROID ? 0.5 : 0.75;
const HIGH_PRIORITY_CACHE_MODULO = IS_SAFARI ? 4 : 30;
const LOW_PRIORITY_CACHE_MODULO = 10;
const KEY_STORE = {};

const sharedWorker = new WorkerConnector(
  new FetchWorker(),
  // new Worker(new URL('../fetch-worker/shared.worker.ts', import.meta.url)),
);
...
const worker = () => {
  ...
  sharedWorker.request(
    {
      name: 'registerWorkerPort',
      args: [
        channel.port2,
      ],
      transfer: [
...
const workers = new Array(MAX_WORKERS).fill(null).map(
  () => new WorkerConnector(worker()),
);
...
class RLottie {
  ...
  constructor (
    ...
    private onLoad?: (self?: RLottie) => void,
    ...
    this.initConfig();
    this.initRenderer();
    // RLottiePlayingObserver.add(this);
  }
  ...
  private initContainer () {
    if (!(this.container.parentNode instanceof HTMLElement)) {
      throw new TypeError('[RLottie] Container is not mounted');
    }
    ...
    if (!size) {
      size = (
        ...
        || Number.parseInt(this.container.style.width, 10)
  ...
  private destroyRenderer () {
    if (!Number.isInteger(this.workerIndex)) {
      return;
    }
