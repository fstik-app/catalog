...
import { throttle, debounce } from './schedulers';
...
export function useIntersectionObserver ({
  ...
  function initController () {
    ...
    const observerCallbackSync = () => {
      const entries = [...entriesAccumulator.values()];
  ...
  const observe = useCallback((target: HTMLElement, targetCallback?: AnyToVoidFunction) => {
    if (!controllerRef.current) {
      initController();
    }
